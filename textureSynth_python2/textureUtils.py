...
def expand(t, f):
    ...
    x1 = mx/2+1 - mx/(2*f)
    x2 = mx/2 + mx/(2*f)
    Te[y1:y2,x1:x2] = T[1:my/f,1:mx/f]
    Te[y1-1,x1:x2] = T[0,1:mx/f] / 2
    Te[y2+1,x1:x2] = np.conj(T[0,range(mx/f-1,0,-1)]/2)
    Te[y1:y2,x1-1] = T[1:my/f,0] / 2
    Te[y1:y2,x2+1] = np.conj(T[range(my/f-1,0,-1),0]/2)
    esq = T[0,0] / 4
    Te[y1-1,x1-1] = esq
    Te[y1-1,x2+1] = esq
    Te[y2+1,x1-1] = esq
