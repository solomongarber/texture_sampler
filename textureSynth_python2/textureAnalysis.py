...
def textureAnalysis(im0, Nsc, Nor, Na):
    ...
    rpyr0 = map(lambda i: np.real(pyr0[i]), range(len(pyr0)))
    apyr0 = map(lambda i: np.abs(pyr0[i]), range(len(pyr0)))
    ...
    for nsc in range(0,Nsc):
        ...
        if nsc < Nsc-1:
            ...
            for nor in range(0,Nor):
                ...
                for i in range(parents.shape[0]): parents[i,nor] = tmp1[i]
        else:
        ...
        for i in range(Nor):
            ...
            for j in range(np.prod(pind0[firstBnum+i])):
                cousins[n] = tmp1[j]
                n += 1
                if n == np.prod(cousins.shape): break
            if n == np.prod(cousins.shape): break
        cousins = np.reshape(cousins, [cousinSz, Nor], order='F')
        nc = cousins.shape[1]
        if len(parents) > 0: nq = parents.shape[1]
        else: nq = 0
    ...
    print statg0,len(statg0)
    print statsLPim, len(statsLPim)
    print acr, acr.shape
    print ace, ace.shape
    print magMeans0, magMeans0.shape
    print C0,C0.shape
    print Cx0,Cx0.shape
    print Cr0,Cr0.shape
    print Crx0,Crx0.shape
    print vHPR0
